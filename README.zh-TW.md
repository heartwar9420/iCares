...
3. **次晝夜節律 (Ultradian Rhythms)：** 當累積專注達 100 分鐘後，系統會觸發提醒，建議使用者進行 20 慢速深度休息。
...
## 快速啟動 (Local Development with Docker)

為了確保團隊開發環境的一致性，本專案支援使用 Docker 進行本地部署。透過多容器架構設計，您無需手動安裝 Node.js 或 Python，即可一鍵啟動 Next.js 前端與 FastAPI 後端，並在獨立的虛擬網路中完成 WebSocket 通訊對接。

### 前置作業

請確認您的電腦已安裝 [Docker Desktop](https://www.docker.com/products/docker-desktop/)。
您需要在專案根目錄建立一份 `.env.local` 檔案，並填入您的 Supabase 專案金鑰：
...
**實際解決效果展示：**
_(展示重整網頁或切換背景後，計時器依然精準對齊伺服器時間)_
